...
##### initialize the cumulative matrix with zeros
combo_matrix = pd.DataFrame(0, index=tip_labels_list, columns=tip_labels_list)

##### initialize a count matrix to keep track of non-NaN values used for each element
count_matrix = pd.DataFrame(0, index=tip_labels_list, columns=tip_labels_list)

##### loop through the expanded matrices
for expanded_matrix in expanded_matrices:
    ###### increment the count matrix for non-NaN values in each expanded matrix
    count_matrix += ~expanded_matrix.isna()

    ###### add the expanded matrix values to the cumulative matrix
    combo_matrix += expanded_matrix.fillna(0)

##### compute the cumulative average matrix
combo_matrix = combo_matrix / count_matrix

##### print the cumulative average matrix
#print("combo_matrix:")
#print(combo_matrix)
...
##### convert clusters into a square pirwise distance matrix
...
##### display the lsit of clusters and pairwise distances
...
##### set diagonal of final_pairwise_matrix = 0
